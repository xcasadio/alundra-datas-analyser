...
- `map_N.tmj` : carte Tiled JSON, avec les couches `Ground`, `Walls_*`, `Portals`, `MapEvents` et `Entities`.
...
Le compagnon brut conserve notamment les donnees par cellule (`Walkability`, `GroundProperty`, `Slope`, `Height`, `WallTilesOffset`, `TileId`, `Palette`, `Tile`, `Flags`) et les piles de murs (`Offset`, `Count`, ids de tiles bruts, positions renderer calculees). Les couches `Walls_*` sont placees sur les cellules logiques ; les offsets verticaux exacts restent dans le compagnon.
