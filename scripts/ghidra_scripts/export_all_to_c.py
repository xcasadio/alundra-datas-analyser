...
prefixes_to_exclude = [
     "_", "BIOS_", "Cd", "CD", "CD_", "EVENT_", "EXT_", "FONT_", "GPU_", "INTR_", "ISO9660_", 
    "Krom2RawAdd", "MEM", "PAD", "PRNT_", "PUT", "S_", "SPRINTF_", 
    "SPU_", "SR_", "STR", "SYS_", "Spu", "Ss", "VSYNC", "gte_", "v_", "get_", "set_", "def_",
    "ClearImage", "LoadImage", "StoreImage", "MoveImage", "SetDrawArea",
    "GetODE", "SetTexWindow", "SetDrawArea", "SetDrawOffset", "SetPriority", "SetDrawMode",
    "callback", "PC", "Fnt"
]

contains_to_exclude = ["Intr", "DispEnv", "DrawEnv", "OTag", "Sync", "Graph", "RCnt", "Prim"]

for func in functions:
    try:
        func_name = func.getName()

        if any(func_name.startswith(prefix) for prefix in prefixes_to_exclude):
            print("Exclude function (1): {}".format(func_name))
            continue

        if any(word in func_name for word in contains_to_exclude):
            print("Exclude function (2): {}".format(func_name))
            continue
