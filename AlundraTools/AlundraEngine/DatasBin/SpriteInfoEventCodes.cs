...
public class SpriteInfoEventCodes
{
    ...
    public static readonly Dictionary<byte, string> CommandNameByCode = new()
    {
        ...
        { 0x3C, "If at least one entity inside zone + flags & 0x80" },
        { 0x3D, "If at least one entity inside zone" },
        { 0x3E, "Is player riding entity" },
        { 0x3F, "If an entity riding me" },
        { 0x40, "Set program index" },
        { 0x41, "Set sprite program index" },
        { 0x42, "LogicContextEntity = PlayerEntity" },
        { 0x43, "LogicContextEntity = matching entity by function id" },
        ...
        { 0x4E, "Set debug flag" },
        { 0x4F, "ActivateDebugTextAutoAdvance" },
        ...
        { 0x54, "Set walkable" },
        { 0x55, "Set unwalkable" },
        { 0x56, "Change area tiles" },
        { 0x57, "Goto by animation direction" },
        { 0x58, "Directional branch" },
        { 0x59, "Set entity anim" },
        { 0x5A, "Turn entity" },
        { 0x5B, "Turn entity with anim" },
        { 0x5C, "Dialog with entity" },
        { 0x5D, "Set entities.Status = 3" },
        { 0x5E, "Set forceZ for entities found with function id" },
        { 0x5F, "??? 0x5F" },
        { 0x60, "??? 0x60" },
        { 0x61, "??? 0x61" },
        ...
        { 0x66, "Copy logicContext and assign script" },
        { 0x67, "Camera follow entity" },
        { 0x68, "Camera stop follow entity" },
        { 0x69, "Camera look at" },
        { 0x6A, "??? 0x6A" },
        { 0x6B, "??? 0x6B" },
        { 0x6C, "??? 0x6C" },
        ...
        { 0x84, "Use item" },
        { 0x85, "Set map tiles" },
        { 0x86, "Entities.DamagedTickCounter += value" },
        { 0x87, "If entities.TouchingEntity.BalanceAnimValRef.Val == expectedNibble" },
        { 0x88, "??? 0x88" },
        { 0x89, "Move entities" },
        { 0x8A, "Spawn entity" },
        { 0x8B, "Spawn entity according to entity position" },
        { 0x8C, "Random < variable" },
        { 0x8D, "Check PosZ <= TerrainHeight + 1 for entities found with function id" },
        { 0x8E, "Set g_scrollingParameters" },
        { 0x8F, "Set g_scrollingParameters.Flag = 0" },
        { 0x90, "Create effect" },
        { 0x91, "Disable effect" },
        { 0x92, "Set effect anim" },
        { 0x93, "Set effect pos" },
        { 0x94, "Set effect forces" },
        { 0x95, "Check if entities are inside a hitbox" },
        { 0x96, "Restore HP" },
        { 0x97, "Spend money" },
        { 0x98, "Add money" },
        ...
        { 0xA4, "Set scrolling mode" },
        { 0xA5, "Stop all sound" },
        { 0xA6, "Load bgm" },
        { 0xA7, "Play music" },
        { 0xA8, "Is sound loading" },
        { 0xA9, "??? 0xA9" },
        { 0xAA, "??? 0xAA" },
        { 0xAB, "Play sound effect with tone volume mix" },
        ...
        { 0xB0, "Set warpFadeColorRGB and FadeDuration" },
        { 0xB1, "Check if g_fadeStepFlags == 0 && g_warpFlags == 0" },
        { 0xB2, "Check if entities group1 intersect group2" },
        { 0xB3, "Set pad 1 buttons" },
        { 0xB4, "Check if entities have ForceZ < 0" },
        { 0xB5, "Check if entities have ForceZ >= 0" },
        { 0xB6, "Check if entities have ForceZ != 0" },
        { 0xB7, "Check if entities have TargetAnimation == var[2]" },
        { 0xB8, "Check if entities have TargetDirection == var[2]" },
        { 0xB9, "Start cd streaming" },
        { 0xBA, "Check if loading from cd is in progress" },
        { 0xBB, "Check if retry after death or go to title screen" },
    ...
    private static string CreateDescription(byte code)
    {
        var handlers =  GetFunctionNameByCodes();
    ...
    public static readonly Dictionary<byte, CommandProperties> CommandPropertiesByCode = new()
    {
        ...
        {0x0A, new(0x0A, CommandSizeByCode[0x0A], CommandNameByCode[0x0A], CreateDescription(0x0A)) },
        ...
        {0x0D, new(0x0D, CommandSizeByCode[0x0D], CommandNameByCode[0x0D], CreateDescription(0x0D)) },
        ...
        {0x12, new(0x12, CommandSizeByCode[0x12], CommandNameByCode[0x12], CreateDescription(0x12)) },
        {0x15, new(0x15, CommandSizeByCode[0x15], CommandNameByCode[0x15], CreateDescription(0x15)) },
        {0x16, new(0x16, CommandSizeByCode[0x16], CommandNameByCode[0x16], CreateDescription(0x16)) },
        {0x17, new(0x17, CommandSizeByCode[0x17], CommandNameByCode[0x17], CreateDescription(0x17)) },
        ...
        {0x1B, new(0x1B, CommandSizeByCode[0x1B], CommandNameByCode[0x1B], CreateDescription(0x1B)) },
        ...
        {0x1E, new(0x1E, CommandSizeByCode[0x1E], CommandNameByCode[0x1E], CreateDescription(0x1E)) },
        {0x1F, new(0x1F, CommandSizeByCode[0x1F], CommandNameByCode[0x1F], CreateDescription(0x1F)) },
        {0x24, new(0x24, CommandSizeByCode[0x24], CommandNameByCode[0x24], CreateDescription(0x24)) },
        ...
        {0x28, new(0x28, CommandSizeByCode[0x28], CommandNameByCode[0x28], CreateDescription(0x28)) },
        {0x29, new(0x29, CommandSizeByCode[0x29], CommandNameByCode[0x29], CreateDescription(0x29)) },
        {0x2A, new(0x2A, CommandSizeByCode[0x2A], CommandNameByCode[0x2A], CreateDescription(0x2A)) },
        {0x2B, new(0x2B, CommandSizeByCode[0x2B], CommandNameByCode[0x2B], CreateDescription(0x2B)) },
        {0x2D, new(0x2D, CommandSizeByCode[0x2D], CommandNameByCode[0x2D], CreateDescription(0x2D)) },
        ...
        {0x32, new(0x32, CommandSizeByCode[0x32], CommandNameByCode[0x32], CreateDescription(0x32)) },
        ...
        {0x35, new(0x35, CommandSizeByCode[0x35], CommandNameByCode[0x35], CreateDescription(0x35)) },
        {0x36, new(0x36, CommandSizeByCode[0x36], CommandNameByCode[0x36], CreateDescription(0x36)) },
        ...
        {0x39, new(0x39, CommandSizeByCode[0x39], CommandNameByCode[0x39], CreateDescription(0x39)) },
        ...
        {0x45, new(0x45, CommandSizeByCode[0x45], CommandNameByCode[0x45], CreateDescription(0x45)) },
        {0x46, new(0x46, CommandSizeByCode[0x46], CommandNameByCode[0x46], CreateDescription(0x46)) },
        {0x49, new(0x49, CommandSizeByCode[0x49], CommandNameByCode[0x49], CreateDescription(0x49)) },
        ...
        {0x4C, new(0x4C, CommandSizeByCode[0x4C], CommandNameByCode[0x4C], CreateDescription(0x4C)) },
        {0x4D, new(0x4D, CommandSizeByCode[0x4D], CommandNameByCode[0x4D], CreateDescription(0x4D)) },
        ...
        {0x5B, new(0x5B, CommandSizeByCode[0x5B], CommandNameByCode[0x5B], CreateDescription(0x5B)) },
