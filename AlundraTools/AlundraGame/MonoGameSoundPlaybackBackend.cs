...
public sealed class MonoGameSoundPlaybackBackend : ISoundPlaybackBackend
{
    ...
    public bool Play(Stream stream, int voiceId, bool shouldLoop)
    {
        if (stream.CanSeek)
        {
            stream.Position = 0;
        ...
        if (TryCreateDynamicInstance(waveData, shouldLoop, out var dynamicInstance, out var dynamicBufferNeededHandler))
        {
            if ((uint)voiceId < (uint)_voiceInstances.Length)
            {
                ...
                {
                    _voiceBasePitches[voiceId] = _voicePitches[voiceId];
    ...
    private static bool TryCreateDynamicInstance(byte[] waveData, bool shouldLoop, out SoundEffectInstance? instance, out EventHandler<EventArgs>? dynamicBufferNeededHandler)
    {
        instance = null;
